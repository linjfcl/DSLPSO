function SLPSO(Para, Data, qtime, name, maxEvaluations)

    record   = zeros(1, Para.totaltime); 
    record0  = zeros(1, Para.totaltime); 
    initFes  = zeros(1, Para.totaltime);
    
    FbestRec = zeros(Para.totaltime, maxEvaluations);

    RA = zeros(Para.totaltime, maxEvaluations);
    ...
    for run = 1 : Para.totaltime
        ...
        while C~=0
            ...
            initFes(run) = evaluations;
            if(initFes(run) > maxEvaluations)
                disp("error")
                break;
            end
        end
        ...
        while(evaluations < maxEvaluations) 
            ...
            cUnit = maxEvaluations/quench; 
            R = (Rmax/2^(floor(evaluations/cUnit))) * mod((maxEvaluations - evaluations), cUnit) / cUnit;
            ...
            if(rc > 0)
                ...
                for i = NS:-1:1
                    ...
                    if(Fbest >= fval)
                        ...
                        Fbest = fval;
                        UAVPosition = offpop;
                        FbestRec(run, evaluations) = Fbest;
                        RA(run, evaluations) = R(1);
                    end
                    evaluations = evaluations + 1;
                end
                if(rf == 0)
